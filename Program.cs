...
public class Program {
    public static void Main(string[] args) {
        ...
        while (isProgramRunning) {
            ...
            switch (userOption) {
                case "1":
                    AddEmployeeRecord(manager);
                    break;
                case "2":
                    EditEmployeeRecord(manager);
                    break;
                case "3":
                    DeleteEmployeeRecord(manager);
                    break;
                case "4":
    ...
    public static void DeleteEmployeeRecord(EmployeeManager manager) {
    ...
    public static void SearchEmployee(EmployeeManager manager) {
        ...
        Console.WriteLine($"Status: {emp.EmployeeStatus}");
        Console.WriteLine($"Rate Per Hour: PHP {emp.RatePerHr}");
        Console.WriteLine($"Hours Worked: {emp.HoursWorked}");
        Console.WriteLine($"Date Hired: {emp.DateHired}");
        Console.WriteLine($"Gross Pay: PHP {emp.GrossPay}");
        Console.WriteLine($"Overtime Pay: PHP {emp.OvertimePay}");
        Console.WriteLine($"Total Pay: PHP {emp.TotalPay}\n");
    }
}
...
public class Employee {
    ...
    public Employee(int id, string name, string department, string status, double rate, double hours, string dateHired) {
        EmployeeID = id;
        EmployeeName = name.Trim();
        Department = department.Trim();
        EmployeeStatus = status.Trim();
        RatePerHr = rate;
        HoursWorked = hours;
    ...
    public void RecalculateSalary() {
        double baseMultiplier = EmployeeStatus.Equals("Permanent", StringComparison.OrdinalIgnoreCase) ? 1.2 :
                               EmployeeStatus.Equals("Probational", StringComparison.OrdinalIgnoreCase) ? 1.1 : 1.0;

        double overtimeRate = EmployeeStatus.Equals("Permanent", StringComparison.OrdinalIgnoreCase) ? 2.0 :
                              EmployeeStatus.Equals("Probational", StringComparison.OrdinalIgnoreCase) ? 1.75 : 1.5;

        GrossPay = HoursWorked * RatePerHr * baseMultiplier;
        double overtimeHours = Math.Max(0, HoursWorked - 40);
        OvertimePay = overtimeHours * RatePerHr * overtimeRate;
